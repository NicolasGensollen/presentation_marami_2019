...
\begin{frame}
	...
	\medskip
	\onslide<2->{\\{\small A transaction $\left(t,u,v\right) $: \textit{Entity $u$ sends money to entity $v$ at time $t$}.\\Transaction link stream: $ \mathcal{T} = \left( T, V, E_{\mathcal{T}} \right) $}}
	\only<3>{
	\bigskip
	\begin{center}
		\begin{figure}
...
\begin{frame}
	...
	\medskip
	\onslide<2->{\\{\small A certification $\left(t,u,v\right) $: \textit{Member $u$ certifies identity of members $v$ at time $t$}.\\Transaction link stream: $ \mathcal{C} = \left( T, V, E_{\mathcal{C}} \right) $}}
	\bigskip
	\only<3>{
	\begin{center}
		\begin{figure}
